...
def read_ops_from_csv(
    ...
    """
    Reads operations from CSV.

    Required columns (header matching is flexible):
      - Environment
      - SafeName
      - Existing group name (e.g., ExistingSecurityGroupName / OldGroup)
      - New group name (e.g., NewSecurityGroupName / NewGroup)

    Notes:
      - Default encoding is utf-8-sig to gracefully handle UTF-8 with BOM.
      - Use --csv-delimiter ';' if your CSV is semicolon-delimited.
    """
    with open(path, "r", encoding=encoding, newline="") as f:
        ...
        col_env = pick("Environment", "Env")
        col_safe = pick("SafeName", "Safe", "SafeUrlId")
        col_old = pick("ExistingSecurityGroupName", "ExistingGroupName", "OldGroup", "Existing", "CurrentGroup")
        col_new = pick("NewSecurityGroupName", "NewGroupName", "NewGroup", "New", "TargetGroup")

        # Optional custom cols you mentioned:
        col_approver = pick("costco-object-approver", "costcoobjectapprover", "objectapprover")
        col_author = pick("costco-object-author-sme", "costcoobjectauthorsme", "objectauthorsme")

        missing = []
        if col_env is None:
            missing.append("Environment")
        if col_safe is None:
            missing.append("SafeName")
        if col_old is None:
            missing.append("Existing group name")
        if col_new is None:
            missing.append("New group name")
        if missing:
            raise ValueError(f"Missing required column(s) in CSV: {', '.join(missing)}")

        ops: List[RenameOp] = []
        for line_no, row in enumerate(reader, start=2):  # header is line 1
            env = _get_cell_str(row.get(col_env))
            safe = _get_cell_str(row.get(col_safe))
            oldg = _get_cell_str(row.get(col_old))
            newg = _get_cell_str(row.get(col_new))
            ...
            ops.append(
                RenameOp(
                    ...
                    old_group=oldg,
                    new_group=newg,
                    costco_object_approver=(row.get(col_approver) if col_approver else None),
                    costco_object_author_sme=(row.get(col_author) if col_author else None),
                    rownum=line_no,
                )
            )
        return ops
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument("--info", action="store_true", help="Print input summary and exit.")

    # Input
    p.add_argument("--input", required=True, help="Path to CSV file containing operations.")
    p.add_argument("--csv-delimiter", default=",", help="CSV delimiter. Default: ,")
    p.add_argument("--csv-encoding", default="utf-8-sig", help="CSV encoding. Default: utf-8-sig (handles BOM).")
    ...
    ops = read_ops_from_csv(args.input, args.environment, args.csv_delimiter, args.csv_encoding)
