...
.PARAMETER EmitRollback
  If set, writes rollback artifacts (CSV plan + PS1 script) for performed changes.
...
function New-RollbackPs1 {
  ...
  $lines.Add('Runs steps in reverse order of the original changes.')
  $lines.Add('#>')
  $lines.Add('')
  $lines.Add('param(')
  $lines.Add('  [Parameter(Mandatory=$true)][string]$PVWA,')
  $lines.Add('  [Parameter(Mandatory=$true)][string]$Token,')
  ...
  $lines.Add('    $cmd=Get-Command Invoke-RestMethod -EA SilentlyContinue')
  $lines.Add('    if($cmd -and $cmd.Parameters.ContainsKey("SkipCertificateCheck")){ $s["SkipCertificateCheck"]=$true }')
  $lines.Add('    else{ [System.Net.ServicePointManager]::ServerCertificateValidationCallback={ $true } }')
  $lines.Add('  }')
...
foreach ($op in $ops) {
  ...
  try {
    ...
    if ($null -eq $oldMember) {
      $summary.MissingOld++
      if ($AllowMissingOld) {
        Write-Log WARN ("MISSING_OLD: safe={0} member={1} (skipped)" -f $op.SafeName, $op.OldGroup)
        continue
      }
      Write-Log WARN ("MISSING_OLD: safe={0} member={1} (skipped; non-fatal behavior)" -f $op.SafeName, $op.OldGroup)
      continue
    }
...
if ($expiration -is [int] -and $expiration -gt 0) {
  $addBody.membershipExpirationDate = $expiration
    ...
    # capture "new before" for rollback if we update it
    ...
    $summary.Errors++
    Write-Log ERROR ("ERROR: env={0} safe={1} old={2} new={3} row={4} :: {5}" -f $op.Environment, $op.SafeName, $op.OldGroup, $op.NewGroup, $op.RowNum, $_.Exception.Message)
    continue
  }
}
...
if ($EmitRollback) {
  if ($DryRun) {
    Write-Log INFO "EmitRollback requested, but DryRun is enabled. No rollback artifacts written."
  } elseif ($rollbackSteps.Count -eq 0) {
