...
function New-RollbackPs1 {
  ...
  $lines.Add('  if (-not $Script:IRM_SkipCert) {')
  $lines.Add('    [System.Net.ServicePointManager]::ServerCertificateValidationCallback = { $true }')
  $lines.Add('  }')
  $lines.Add('}')
  $lines.Add('')
  $lines.Add('# ---------------------------')
  ...
  $lines.Add('  if ([string]::IsNullOrWhiteSpace($user) -or [string]::IsNullOrWhiteSpace($plain)) {')
  $lines.Add('    throw "AIM returned blank username/password. Line: $line"')
  $lines.Add('  }')
  ...
  $lines.Add('  # logon does not require Authorization header')
