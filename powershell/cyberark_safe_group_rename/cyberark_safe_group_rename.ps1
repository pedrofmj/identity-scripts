...
function New-RollbackPs1 {
  ...
  $lines.Add('')
  $lines.Add('FAULT TOLERANCE')
  $lines.Add('--------------')
  $lines.Add('This rollback script retries transient failures (429/5xx/network/Vault terminated) with exponential backoff + jitter.')
  $lines.Add('Optionally re-logons once on 401/403 (token expired/invalid) if credentials are available.')
  $lines.Add('#>')
  $lines.Add('')
  ...
  $lines.Add('')
  $lines.Add('  # Fault tolerance')
  $lines.Add('  [int]$TimeoutSec = 30,')
  $lines.Add('  [int]$Retries = 5,')
  $lines.Add('  [double]$RetryBackoffSec = 0.75,')
  $lines.Add('  [string]$RetryStatus = "429,500,502,503,504",')
  $lines.Add('  [switch]$ReLogonOnAuthFailure,')
  $lines.Add('')
  ...
  $lines.Add('}')
  $lines.Add('function Parse-RetryStatus([string]$csv){')
  $lines.Add('  $set = New-Object "System.Collections.Generic.HashSet[int]"')
  $lines.Add('  $csv.Split(",") | ForEach-Object { $t=$_.Trim(); if($t){ [void]$set.Add([int]$t) } }')
  $lines.Add('  return $set')
  $lines.Add('}')
  $lines.Add('$Script:RetryStatusSet = Parse-RetryStatus $RetryStatus')
  $lines.Add('')
  $lines.Add('function Get-HttpErrorBody($_err) {')
  $lines.Add('  try {')
  $lines.Add('    $resp = $_err.Exception.Response')
  $lines.Add('    if ($null -eq $resp) { return $null }')
  $lines.Add('    $stream = $resp.GetResponseStream()')
  $lines.Add('    if ($null -eq $stream) { return $null }')
  $lines.Add('    $reader = New-Object System.IO.StreamReader($stream)')
  $lines.Add('    return $reader.ReadToEnd()')
  $lines.Add('  } catch { return $null }')
  $lines.Add('}')
  $lines.Add('')
  $lines.Add('function New-BackoffSeconds([int]$attempt,[double]$base){')
  $lines.Add('  # exp backoff + jitter')
  $lines.Add('  $exp = $base * [math]::Pow(2, $attempt)')
  $lines.Add('  $jitter = (Get-Random -Minimum 0.0 -Maximum ($base))')
  $lines.Add('  return [math]::Min(60.0, ($exp + $jitter))')
  $lines.Add('}')
  $lines.Add('')

  # AIM
  $lines.Add('# ---------------------------')
  $lines.Add('# AIM credential fetch (CLIPasswordSDK)')
  $lines.Add('# ---------------------------')
  ...
  $lines.Add('  $s=@{ Method="POST"; Uri=$u; Headers=@{ "Content-Type"="application/json" }; TimeoutSec=$TimeoutSec; ErrorAction="Stop"; ContentType="application/json"; Body=($body|ConvertTo-Json -Depth 10 -Compress) }')
  ...
  $lines.Add('  try {')
  $lines.Add('    $s=@{ Method="POST"; Uri=$u; Headers=@{ "Content-Type"="application/json"; "Authorization"=$Tok }; TimeoutSec=$TimeoutSec; ErrorAction="Stop" }')
  $lines.Add('    if($NoVerifyTls -and $Script:IRM_SkipCert){ $s["SkipCertificateCheck"]=$true }')
  ...
  # Ensure token + remember whether we logged on in this run
  $lines.Add('# ---------------------------')
  $lines.Add('# Ensure Token (generate from scratch if needed)')
  $lines.Add('# ---------------------------')
  $lines.Add('$didLogonHere = $false')
  $lines.Add('$script:CachedPlainPass = $null')
  $lines.Add('$script:CachedUser = $null')
  $lines.Add('')
  $lines.Add('function Ensure-Token([switch]$Force){')
  $lines.Add('  if((-not $Force) -and (-not [string]::IsNullOrWhiteSpace($Token))){ return }')
  $lines.Add('')
  $lines.Add('  # If no password, try AIM')
  ...
  $lines.Add('  $script:CachedPlainPass = $plainPass')
  $lines.Add('  $script:CachedUser = $Username')
  ...
  $lines.Add('}')
  $lines.Add('')
  $lines.Add('# Default: enable relogon-on-auth-failure unless explicitly disabled')
  $lines.Add('if(-not $PSBoundParameters.ContainsKey("ReLogonOnAuthFailure")){ $ReLogonOnAuthFailure = $true }')
  $lines.Add('')
  $lines.Add('Ensure-Token')
  $lines.Add('')

  # Fault-tolerant Invoke wrapper
  $lines.Add('# ---------------------------')
  $lines.Add('# Fault-tolerant PVWA Invoke')
  $lines.Add('# ---------------------------')
  $lines.Add('function Invoke-Pvwa {')
  $lines.Add('  param(')
  $lines.Add('    [Parameter(Mandatory=$true)][ValidateSet("GET","POST","PUT","DELETE")][string]$Method,')
  $lines.Add('    [Parameter(Mandatory=$true)][string]$Uri,')
  $lines.Add('    $Body')
  $lines.Add('  )')
  $lines.Add('')
  $lines.Add('  $attempt = 0')
  $lines.Add('  $reLogged = $false')
  $lines.Add('  while($attempt -le $Retries){')
  $lines.Add('    try {')
  $lines.Add('      $hdr = @{ "Content-Type"="application/json" }')
  $lines.Add('      if(-not [string]::IsNullOrWhiteSpace($Token)){ $hdr["Authorization"] = $Token }')
  $lines.Add('      $s = @{ Method=$Method; Uri=$Uri; Headers=$hdr; TimeoutSec=$TimeoutSec; ErrorAction="Stop" }')
  $lines.Add('      if($NoVerifyTls -and $Script:IRM_SkipCert){ $s["SkipCertificateCheck"]=$true }')
  $lines.Add('      if($null -ne $Body){')
  $lines.Add('        $s["ContentType"]="application/json"')
  $lines.Add('        $s["Body"]=($Body|ConvertTo-Json -Depth 30 -Compress)')
  $lines.Add('      }')
  $lines.Add('      return Invoke-RestMethod @s')
  $lines.Add('    } catch {')
  $lines.Add('      $err = $_')
  $lines.Add('      $status = $null')
  $lines.Add('      try { if($err.Exception.Response -and $err.Exception.Response.StatusCode){ $status = [int]$err.Exception.Response.StatusCode } } catch {}')
  $lines.Add('      $bodyText = Get-HttpErrorBody $err')
  $lines.Add('      $msg = $err.Exception.Message')
  $lines.Add('')
  $lines.Add('      # Some PVWA/Vault transient errors appear only in body text')
  $lines.Add('      $isVaultTerminated = $false')
  $lines.Add('      if($bodyText -match "GENWF014E" -or $bodyText -match "terminated" -or $msg -match "GENWF014E" -or $msg -match "terminated"){ $isVaultTerminated = $true }')
  $lines.Add('')
  $lines.Add('      $isAuthFail = ($status -eq 401 -or $status -eq 403)')
  $lines.Add('      if($isAuthFail -and $ReLogonOnAuthFailure -and (-not $reLogged)){')
  $lines.Add('        try {')
  $lines.Add('          Write-Host ("AUTH failure ({0}). Re-logging in once and retrying..." -f $status) -ForegroundColor Yellow')
  $lines.Add('          # force re-logon if we have cached creds or can retrieve via AIM/prompt')
  $lines.Add('          $Token = $null')
  $lines.Add('          Ensure-Token -Force')
  $lines.Add('          $reLogged = $true')
  $lines.Add('          continue')
  $lines.Add('        } catch {')
  $lines.Add('          throw $err')
  $lines.Add('        }')
  $lines.Add('      }')
  $lines.Add('')
  $lines.Add('      $retryable = $false')
  $lines.Add('      if($null -ne $status -and $Script:RetryStatusSet.Contains($status)){ $retryable = $true }')
  $lines.Add('      if($isVaultTerminated){ $retryable = $true }')
  $lines.Add('      if($msg -match "(?i)timed out|timeout|temporarily unavailable|connection.*(closed|reset)|The underlying connection was closed|Unable to connect|The remote name could not be resolved"){ $retryable = $true }')
  $lines.Add('')
  $lines.Add('      if($attempt -lt $Retries -and $retryable){')
  $lines.Add('        $sleep = New-BackoffSeconds -attempt $attempt -base $RetryBackoffSec')
  $lines.Add('        $detail = ""')
  $lines.Add('        if($null -ne $status){ $detail = "status=$status" }')
  $lines.Add('        elseif($isVaultTerminated){ $detail = "vault-terminated" }')
  $lines.Add('        Write-Host ("Retryable failure on {0} {1} ({2}). Sleeping {3:N2}s then retry..." -f $Method,$Uri,$detail,$sleep) -ForegroundColor Yellow')
  $lines.Add('        Start-Sleep -Seconds $sleep')
  $lines.Add('        $attempt++')
  $lines.Add('        continue')
  $lines.Add('      }')
  $lines.Add('')
  $lines.Add('      if($bodyText){')
  $lines.Add('        throw ("PVWA call failed: " + $msg + " :: BODY=" + ($bodyText -replace "`r?`n"," "))')
  $lines.Add('      }')
  $lines.Add('      throw $err')
  $lines.Add('    }')
  $lines.Add('  }')
  ...
  # Rollback operations (use Invoke-Pvwa)
  $lines.Add('# ---------------------------')
  $lines.Add('# Rollback operations')
  $lines.Add('# ---------------------------')
  $lines.Add('function AddMember($safe,$body){')
  $lines.Add('  $u="$PVWA/PasswordVault/API/Safes/$(Escape-Url $safe)/Members/"')
  $lines.Add('  Invoke-Pvwa -Method "POST" -Uri $u -Body $body | Out-Null')
  $lines.Add('}')
  ...
  $lines.Add('  Invoke-Pvwa -Method "PUT" -Uri $u -Body $body | Out-Null')
  $lines.Add('}')
  $lines.Add('function DeleteMember($safe,$member){')
  $lines.Add('  $u="$PVWA/PasswordVault/API/Safes/$(Escape-Url $safe)/Members/$(Escape-Url $member)/"')
  $lines.Add('  try{ Invoke-Pvwa -Method "DELETE" -Uri $u -Body $null | Out-Null } catch { }')
  $lines.Add('}')
  $lines.Add('')
  ...
  for ($i = $stepsArr.Count - 1; $i -ge 0; $i--) {
    ...
    if ($action -eq "ADD_OLD_BACK") {
      ...
      # robust JSON embedding (avoid PS object literal quirks in emitted script)
