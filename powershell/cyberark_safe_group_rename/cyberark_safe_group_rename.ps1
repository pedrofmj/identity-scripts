...
function New-RollbackPs1 {
  ...
  # IRM wrapper (safe: named params + splatting hashtable)
  $lines.Add('function IRM {')
  $lines.Add('  param(')
  $lines.Add('    [Parameter(Mandatory=$true)][ValidateSet("GET","POST","PUT","DELETE")]')
  $lines.Add('    [string]$Method,')
  $lines.Add('')
  $lines.Add('    [Parameter(Mandatory=$true)]')
  $lines.Add('    [string]$Uri,')
  $lines.Add('')
  $lines.Add('    $Body')
  $lines.Add('  )')
  $lines.Add('')
  $lines.Add('  $headers = Get-Headers $Token')
  $lines.Add('  $splat = @{ Method = $Method; Uri = $Uri; Headers = $headers; ErrorAction = "Stop" }')
  $lines.Add('  if ($NoVerifyTls -and $Script:IRM_SkipCert) { $splat["SkipCertificateCheck"] = $true }')
  $lines.Add('  if ($null -ne $Body) {')
  $lines.Add('    $splat["ContentType"] = "application/json"')
  $lines.Add('    $splat["Body"]        = ($Body | ConvertTo-Json -Depth 30 -Compress)')
  $lines.Add('  }')
  $lines.Add('  Invoke-RestMethod @splat')
  $lines.Add('}')
  $lines.Add('')
