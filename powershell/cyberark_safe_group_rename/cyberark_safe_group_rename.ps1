...
function New-RollbackPs1 {
  ...
  $lines.Add('')
  $lines.Add('  # Fault tolerance / retry')
  $lines.Add('  [int]$TimeoutSec = 30,')
  $lines.Add('  [int]$Retries = 6,')
  $lines.Add('  [double]$RetryBackoffSec = 0.75,')
  $lines.Add('  [string]$RetryStatus = "429,500,502,503,504",')
  $lines.Add('  # Retry on PVWA/Vault transient patterns (ex: CAWS00001E / GENWF014E Connection to the Vault was terminated)')
  $lines.Add('  [string]$RetryBodyRegex = "(?i)CAWS00001E|GENWF014E|Connection to the Vault was terminated|temporarily unavailable|timeout|timed out|connection.*(closed|reset)",')
  $lines.Add('')
  ...
  # Fault-tolerant IRM wrapper (retry + body-pattern retry)
  $lines.Add('function IRM {')
  $lines.Add('  param(')
  ...
  $lines.Add('    [object]$Body')
  $lines.Add('  )')
  $lines.Add('')
  $lines.Add('  $attempt = 0')
  $lines.Add('  $lastErr = $null')
  $lines.Add('')
  ...
  $lines.Add('        Uri         = $Uri')
  $lines.Add('        Headers     = (Get-Headers $Token)')
  $lines.Add('        TimeoutSec  = $TimeoutSec')
  ...
  $lines.Add('      $lastErr = $_')
  $lines.Add('')
  $lines.Add('      $statusCode = $null')
  $lines.Add('      try { $resp = $_.Exception.Response; if($resp -and $resp.StatusCode){ $statusCode = [int]$resp.StatusCode } } catch {}')
  $lines.Add('      $errBody = Get-HttpErrorBody $_')
  $lines.Add('')
  $lines.Add('      $retryable = $false')
  $lines.Add('      if ($null -ne $statusCode -and $Script:RetryStatusSet.Contains($statusCode)) { $retryable = $true }')
  $lines.Add('      if ($errBody -and ($errBody -match $RetryBodyRegex)) { $retryable = $true }')
  $lines.Add('')
  $lines.Add('      $ex = $_.Exception')
  $lines.Add('      if ($ex -ne $null) {')
  $lines.Add('        $t = $ex.GetType().FullName')
  $lines.Add('        if ($t -eq "System.Net.Http.HttpRequestException") { $retryable = $true }')
  $lines.Add('        elseif ($t -eq "System.Net.WebException") { $retryable = $true }')
  $lines.Add('        elseif ($ex.Message -match $RetryBodyRegex) { $retryable = $true }')
  $lines.Add('        elseif ($ex.InnerException -ne $null -and $ex.InnerException.Message -match $RetryBodyRegex) { $retryable = $true }')
  $lines.Add('      }')
  $lines.Add('')
  $lines.Add('      if ($attempt -lt $Retries -and $retryable) {')
  $lines.Add('        $sleep = [math]::Min(60, ($RetryBackoffSec * [math]::Pow(2, $attempt)))')
  $lines.Add('        $msg = $_.Exception.Message')
  $lines.Add('        if ($errBody) { $msg = ($msg + " | PVWA_BODY=" + ($errBody -replace "\r?\n"," ")) }')
  $lines.Add('        Write-Host ("[WARN] IRM retry {0}/{1}: {2} {3} (status={4}) sleep={5:N2}s :: {6}" -f ($attempt+1), ($Retries+1), $Method, $Uri, $statusCode, $sleep, $msg)')
  $lines.Add('        Start-Sleep -Seconds $sleep')
  $lines.Add('        $attempt++')
  $lines.Add('        continue')
  $lines.Add('      }')
  $lines.Add('')
  $lines.Add('      if ($errBody) {')
  $lines.Add('        throw ("IRM failed: {0} {1} (status={2}) :: {3} :: PVWA_BODY={4}" -f $Method, $Uri, $statusCode, $_.Exception.Message, ($errBody -replace "\r?\n"," "))')
  $lines.Add('      }')
  $lines.Add('      throw $lastErr')
  $lines.Add('    }')
  $lines.Add('  }')
  $lines.Add('  throw $lastErr')
  $lines.Add('}')
  ...
  $lines.Add('  $s=@{ Method="POST"; Uri=$u; Headers=@{ "Content-Type"="application/json" }; TimeoutSec=$TimeoutSec; ErrorAction="Stop" }')
  $lines.Add('  if($NoVerifyTls -and $Script:IRM_SkipCert){ $s["SkipCertificateCheck"]=$true }')
  ...
  $lines.Add('  try {')
  $lines.Add('    $s=@{ Method="POST"; Uri=$u; Headers=(Get-Headers $Tok); TimeoutSec=$TimeoutSec; ErrorAction="Stop" }')
  $lines.Add('    if($NoVerifyTls -and $Script:IRM_SkipCert){ $s["SkipCertificateCheck"]=$true }')
  ...
  # Rollback operations (now calling IRM with named params)
  $lines.Add('function AddMember($safe,$body){')
  $lines.Add('  $u="$PVWA/PasswordVault/API/Safes/$(Escape-Url $safe)/Members/"')
  $lines.Add('  IRM -Method "POST" -Uri $u -Body $body | Out-Null')
  $lines.Add('}')
  ...
  $lines.Add('  IRM -Method "PUT" -Uri $u -Body $body | Out-Null')
  $lines.Add('}')
  $lines.Add('function DeleteMember($safe,$member){')
  $lines.Add('  $u="$PVWA/PasswordVault/API/Safes/$(Escape-Url $safe)/Members/$(Escape-Url $member)/"')
  $lines.Add('  try{ IRM -Method "DELETE" -Uri $u -Body $null | Out-Null } catch { }')
  $lines.Add('}')
  $lines.Add('')
