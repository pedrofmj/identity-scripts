  ...
  while ($attempt -le $Retries) {
    try {
      ...
      if ($null -ne $statusCode -and $Script:RetryStatusSet.Contains($statusCode)) { $retryable = $true }
            # Some PowerShell/.NET runtimes don't have System.Net.Http.HttpRequestException type loaded,
            # so avoid hard-referencing the type and instead compare by name.
            $ex = $_.Exception
            if ($ex -ne $null) {
